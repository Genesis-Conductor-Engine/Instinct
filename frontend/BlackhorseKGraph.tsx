...
// Types from ʞ Graph + Blackhorse integration
...
const BlackhorseKGraphDashboard: React.FC = () => {
  ...
  // Initialize WebSocket connection to Blackhorse/ʞGraph bridge
  useEffect(() => {
  ...
  // Cytoscape elements for knowledge graph visualization
  const cyElements = React.useMemo(() => {
    const nodes = knowledgeNodes.map(n => ({
      ...
      style: {
        'background-color': n.type === 'threat' ? '#ef4444' :
                           n.type === 'mitigation' ? '#22c55e' :
                           n.type === 'event' ? '#f59e0b' : '#3b82f6',
        width: 40 + n.confidence * 20,
        height: 40 + n.confidence * 20,
        'border-width': n.extracted_by === 'Opus' ? 4 :
                       n.extracted_by === 'Sonnet' ? 2 : 1,
        'border-color': '#ffffff'
      }
    }));

    // Generate edges based on temporal proximity and shared IPs
    const edges: any[] = [];
    for (let i = 0; i < knowledgeNodes.length - 1; i++) {
      for (let j = i + 1; j < Math.min(i + 5, knowledgeNodes.length); j++) {
  ...
  // Cost trajectory data for charts
  ...
  const requestModalityEscalation = useCallback(() => {
    if (socket) {
      socket.emit('modality_request', {
        target:
          modality.current_model === 'Haiku'
            ? 'Sonnet'
            : modality.current_model === 'Sonnet'
              ? 'Opus'
              : 'Haiku',
    ...
    <div className={styles.dashboard}>
      <header className={styles.header}>
        ...
        <div className={styles.statusBar}>
          ...
          <button
            className={styles.glassBreakBtn}
            onClick={triggerGlassBreak}
            disabled={!sovereignty.glass_break_armed}
          >
          ...
          <div className={styles.knowledgePanel}>
            ...
            <div className={styles.extractionStats}>
              ...
              <div className={styles.statCard}>
                ...
                <div className={styles.statValue}>
                  {(
                    (knowledgeNodes.reduce((a, b) => a + b.confidence, 0) / (knowledgeNodes.length || 1)) *
                    100
                  ).toFixed(1)}
                  %
                </div>
                    ...
                    <div className={styles.bar}>
                      <div
                        ...
                        style={{
                          ...
                          background:
                            model === 'Opus' ? '#ef4444' : model === 'Sonnet' ? '#f59e0b' : '#22c55e'
                        }}
                      />
                    </div>
                  </div>
            ...
            <div className={styles.protocolStatus}>
              ...
              <div className={styles.protocolList}>
                <div className={styles.protocolItem}>
                  ...
                  <span
                    className={`${styles.protocolState} ${sovereignty.glass_break_armed ? styles.armed : styles.disarmed}`}
                  >
